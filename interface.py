...
async def startgame(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if 'game' not in context.chat_data or context.chat_data['game']['state'] == -1:
        ...
        if n < 3:
            ...
            game['roles'] = roles
            roles_str = ['狼人', '平民', '先知']
            wolves = ' 和 '.join(game["players"][i]["mention"] for i in game['players'].keys() if game['roles'][i] == 0)
            for player, user_dict in game['players'].items():
                user_dict['game'] = game
                await context.bot.send_message(user_dict['private'], f'你在游戏中的身份是：{roles_str[roles[player]]}。')
                if roles[player] == 0:
                    await context.bot.send_message(user_dict['private'], f'本局游戏中的狼人是 {wolves}。')
    ...
    if game['state'] == 1:
        ...
        for player, user_dict in game['players'].items():
            if game['roles'][player] != 1:
                await context.bot.send_message(user_dict['private'], f'本局游戏的词语是：{game["word"]}。')
...
async def new_question(bot: ExtBot, game: dict) -> None:
    ...
    game['chat_question'] = await bot.send_message(game['chat'], f'请 {game["players"][current]["mention"]} 回复本条消息以提问。', parse_mode=ParseMode.HTML)
...
async def update_host_question(bot: ExtBot, game: dict) -> None:
    ...
    if 'host_question' in game:
        ...
        game['host_question'] = await bot.send_message(game['players'][host]['private'], text, reply_markup=keyboard)
...
async def game_over_correct(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.answer()
    game['state'] = 3
    wolves = ' 和 '.join(game["players"][id]["mention"] for id in game['players'].keys() if game['roles'][id] == 0)
...
async def game_over_correct_force(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
def voting_keyboard(game: dict) -> None:
    return InlineKeyboardMarkup.from_column([InlineKeyboardButton(game['players'][id]['name'], callback_data=f'!{id}') for id in game['candidates']])
...
async def finish_vote(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    board = '\n'.join('%02d 票 %s' % (x, game["players"][id]["name"]) for id, x in zip(c, results))
    ...
    if game['state'] == 3:
        ...
        if results[c.index(id)] > 0:
            await context.bot.send_message(game['chat'], f'狼人成功找到了先知 {game["players"][id]["mention"]}。\n狼人获胜！', parse_mode=ParseMode.HTML)
        else:
            await context.bot.send_message(game['chat'], f'狼人没有找到先知 {game["players"][id]["mention"]}。\n村民获胜！', parse_mode=ParseMode.HTML)
    else:
        # fail
        x = max(results)
        if results.count(x) > 1:
            ...
            if game['roles'][id] == 0:
                ...
                await context.bot.send_message(game['chat'], f'{game["players"][id]["mention"]} 不是狼人。\n狼人是 {wolves}。\n狼人获胜！', parse_mode=ParseMode.HTML)
    game['state'] = -1